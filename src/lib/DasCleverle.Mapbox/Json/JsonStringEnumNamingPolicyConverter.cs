...
internal class JsonStringEnumNamingPolicyConverterAttribute : JsonConverterAttribute
{
    public JsonNamingPolicy NamingPolicy { get; }

    public JsonStringEnumNamingPolicyConverterAttribute(JsonNamingPolicy namingPolicy)
    {
    ...
    {
        var converterType = typeof(JsonStringEnumNamingPolicyConverter.Converter<>).MakeGenericType(typeToConvert);
        return (JsonConverter)Activator.CreateInstance(converterType, new object[] { NamingPolicy })!;
    }
}

internal class JsonStringEnumNamingPolicyConverter : JsonConverterFactory
{
    private readonly JsonNamingPolicy _namingPolicy;

    public JsonStringEnumNamingPolicyConverter(JsonNamingPolicy namingPolicy)
    {
